...
boundary ($P = 0.5$, i.e. $z = 0$) is the perceptron's straight
...
![Left: test-set confusion matrix for the 3×3 perceptrons. Almost all mistakes involve "cross": 9 crosses predicted as "vertical", and 4 horizontals and 2 verticals predicted as "cross". These are the pairs that share pixels. Right: the ROC curve of the "cross" perceptron's score, cross vs. everything else.](figs/day08-3x3-evaluation.png){width=100%}
...
![The architectures still to come, each with a biological application from this course. Feed-forward networks take a fixed-size input (today's localization model; PHD on Day 9). CNNs look for local patterns with shared weights (contact maps, Day 10). RNNs read a sequence one position at a time (signal peptides, Day 11). Transformers let every position attend to every other (language models and SignalP 6.0, Day 11; AlphaFold, Day 12). Generative diffusion models learn to turn noise into new examples (protein design, Day 12).](figs/day08-architectures.png){width=100%}
