...
**Full lab:** follow the notebook to build the localization dataset
yourself:

1. fetch and filter the UniProt entries;
2. balance and one-hot encode them;
3. cluster them and make the homology-aware split;
4. measure how much homology leakage inflates a baseline;
5. define the `nn.Module` and check its output shape and untrained
   metrics.

Stop there. Day 9 is where this model learns.
