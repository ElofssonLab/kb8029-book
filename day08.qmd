...
A PyTorch model is written as a class inheriting from `nn.Module` — Day
2's OOP lesson, now doing real work: a class with its layers as
attributes (set up in `__init__`) and a `forward()` method describing
how input tensors become output tensors. This is the pattern every
model in the rest of this course follows.
...
the above concretely: a train/val/test split demo, plots of sigmoid,
tanh, and ReLU, and the case study — five subcellular-localization
