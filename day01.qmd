...
### Shapes: matrices as data tables

For everything you will do in NumPy and PyTorch, one habit matters more than any formula: **always know the shape of every matrix and vector.** A matrix with $n$ rows and $m$ columns has shape $(n, m)$, also written $n \times m$. In biology, a matrix is almost always a data table:
...
**One matrix–vector product gives 1,000 scores, one per protein.** No loop is needed. This is exactly how Day 6's perceptron and Day 8's classifiers are written: stack every example as a row of $X$ and compute `X @ w`.
